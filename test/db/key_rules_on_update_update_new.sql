...
INSERT INTO key_rules (keys, key_order, promotion, values_comment) VALUES ('test.b*', NULL, 1, 'z');
...
UPDATE key_rules SET (keys, promotion) = ('test.a*', 3) WHERE values_comment = 'z';

SELECT * INTO STRICT result FROM keys WHERE key = 'test.alpha';
PERFORM assert(result.key_order IS NOT DISTINCT FROM 1);
PERFORM assert(result.promotion IS NOT DISTINCT FROM 3);
PERFORM assert(result.key_comment IS NOT DISTINCT FROM 'y');
